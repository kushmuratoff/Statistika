...
aa<-summary(logit)$coefficients[,1]
length(aa)
lg_df
#(Intercept) -4.095339 Chemistry    0.043336 Literature   0.019447  
#Geometry    -0.022803 English      0.012117
Y_hat<-myPrdict(lg_df[-5],aa)
Y_hat
lg_df['Yhat'] <- (Y_hat > 0.5)

yy<-need_df4['Salary'] > 50000
yy[2]
summ<-0
for(i in 1:length(lg_df[,1]))
{
  #print(lg_df[i,'Yhat'],lg_df[i,'Salary'])
  if(lg_df[i,'Yhat'] == yy[i])
  {
    summ<-summ+1
  }
}
print(paste("Tochnost:",summ/length(lg_df[,1]),sep=""))
